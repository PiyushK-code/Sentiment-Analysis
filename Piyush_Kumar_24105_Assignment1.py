# -*- coding: utf-8 -*-
"""Copy of Assignment 1 for DS-207 (Intro to NLP) - Text Classification.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/15CKQ1kRnGXO5ymzs7WPXkaLo00RTNZbx
...
STUDENT_SAP_NAME  = "Piyush Kumar"
STUDENT_SR_NUMBER = "24105"
...
class NaiveBayesClassifier:
    ...
    def create_vocabulary(self, texts):
        ...
        #Tokenize all text documents by using preprocess_text function
        tokens = []
        for doc in texts:
          tokens.extend(self.preprocess_text(doc))
        ...
        # Create vocabulary, include <UNK> token at index 0
        vocabulary = {'<UNK>': 0}
        index = 1

        # Only include words in vocabulary that appears >= min_freq times
        for word, freq in word_freq.items():
          if freq >= self.min_freq:
            vocabulary[word] = index
            index += 1

        # Return vocabulary, word to index mapping
        return vocabulary
    ...
    def extract_features(self, texts, vocabulary):
        ...
        # Feature matrix where each row is a document vector
        feature_matrix = []

        # Iterate over each text document
        for doc in texts:

          # Preprocess the document using preprocess_text function
          tokens = self.preprocess_text(doc)

          # Feature vector for the document with all counts initialized to 0
          feature_vector = [0] * len(vocabulary)

          # Count occurrence of each word in the document, Words not in vocabulary are counted as <UNK>
          for word in tokens:
            index = vocabulary.get(word, 0)
            feature_vector[index] += 1

          # Add document feature vector into feature matrix
          feature_matrix.append(feature_vector)

        # Convert feature matrix into numpy array and return
    ...
    def calculate_class_probabilities(self, y):
        ...
        # Count frequency of each unique class in y
        for cls in y:
        ...
        for cls in class_prob:
          class_prob[cls] = class_prob[cls] / total_samples
    ...
    def calculate_word_probabilities(self, X, y, vocabulary, alpha=1.0):
        ...
        #Initialize nested dict with P(word|class) for each word and class
        word_prob = {cls: {word : 0 for word in vocabulary} for cls in unique_classes}

        for cls in word_prob:
            # Get rows(doc) in X corresponding to the current class
            index = [i for i in range(len(y)) if y[i] == cls]
            X_cur_class = X[index]

            # Calculate word count for the class, Each column represents the freq of occurance of a particular word
            word_count = X_cur_class.sum(axis=0)
            total_words = word_count.sum()

            # Calculate P(word|class) for each word (with Laplace smoothing)
            for word, index in vocabulary.items():
                word_prob[cls][word] = (word_count[index] + alpha) / (total_words + alpha * vocab_size)
    ...
    def predict(self, X_text):
        ...
        for doc in X_text:
            ...
            for cls in self.class_probs:
                # Start with the log of the P(class)
                prob[cls] = np.log(self.class_probs[cls])

                # Add the log probabilities for each word in the doc
                for token in tokens:
                    word = token if token in self.vocabulary else '<UNK>'
                    prob[cls] += np.log(self.word_probs[cls].get(word))

            # Return class with highest log probability score
...
class WordEmbeddingOps:
    ...
    def cosine_similarity(self, vec1, vec2):
        ...
        # Calculate dot product (vec1 · vec2)
        tmp = np.dot(vec1, vec2)

        # Calculate magnitude of vectors ||vec1|| and ||vec2||
        v1 = np.linalg.norm(vec1)
        v2 = np.linalg.norm(vec2)

        # Handle the case when norm of vector is 0
        if v1 == 0 or v2 == 0:
          return 0
    ...
    def euclidean_similarity(self, vec1, vec2):
        ...
        # Convert euclidean distance to similarity as similarity = 1 / (1 + distance)
        similarity = 1 / (1 + euc_distance)
        return similarity
    ...
    def find_analogies(self, word1, word2, word3, similarity_func='cosine', num_results=5):
        ...
        # Use vector arithmetic to compute target word vector: word2 - word1 + word3
        target_word = self.word_vectors[word2] - self.word_vectors[word1] + self.word_vectors[word3]

        if similarity_func == 'cosine':
          sim_func = self.cosine_similarity
        else:
          sim_func = self.euclidean_similarity

        similarity_score = []
        # Iterate through all words in vocabulary to get similarity with target word
        for word in self.word_vectors.index_to_key:

          # Exclude input words from result
          if word not in [word1, word2, word3]:

            # Calculate similarity of word with the target
            cur_word = self.word_vectors[word]
            score = sim_func(target_word, cur_word)

            similarity_score.append((word, score))

        # Return top 'num_results' words with highest similarity score with the target word
        similarity_score.sort(key = lambda x: x[1], reverse=True)
        return similarity_score[:num_results]
    ...
    def find_similar_words(self, word, num_results=5, similarity_func='cosine'):
        ...
        # Return empty list if input word not in vocabulary
        if word not in self.word_vectors:
          return []

        # Get vector representation of given word
        given_word = self.word_vectors[word]
        ...
        similarity_score = []
        # Iterate through all words in vocabulary to get similarity with given word
        for tmp_word in self.word_vectors.index_to_key:

          # Exclude input word from result
          if tmp_word != word:

            # Calculate similarity of all words in vocabulary with the given word
            cur_word = self.word_vectors[tmp_word]
            score = sim_func(given_word, cur_word)

            similarity_score.append((tmp_word, score))

        # Return top 'num_results' words with highest similarity score with the given word
        similarity_score.sort(key = lambda x: x[1], reverse=True)
        return similarity_score[:num_results]
...
class BagOfWordsClassifier:
    ...
    def text_to_bow(self, texts):
        ...
        for doc in texts:
          ...
          # Feature vector for the document with all counts initialized to 0
          feature_vector = [0] * len(self.vocabulary)
...
class Word2VecClassifier:
    ...
    def text_to_vec(self, texts):
        ...
        for doc in texts:
          ...
          words_in_vocab = []
          # Collect vector of words which are present in word2vec vocabulary, rest words are skipped
          for word in tokens:
            if word in self.word_vectors:
              words_in_vocab.append(self.word_vectors[word])

          # Document vector is average of all found word vectors (present in word2vec vocabulary)
          if words_in_vocab:
            avg_vector = np.mean(words_in_vocab, axis=0)
            doc_vectors.append(avg_vector)
          # Documents with no known words get zero vector
          else:
